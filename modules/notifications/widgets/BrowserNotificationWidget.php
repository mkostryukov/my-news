 ...
 *
 * BrowserNotificationsWidget::widget([
 ...
 * </code>
 */
class BrowserNotificationWidget extends Widget
{
    /**
     * Use jQuery Growl
    ...
    {
        $view = $this->getView();
        $asset = BrowserNotificationAsset::register($view);
        ...
        if (!is_null($this->theme)) {
            ...
            foreach (['js' => 'registerJsFile', 'css' => 'registerCssFile'] as $type => $method) {
                $filename = BrowserNotificationAsset::getFilename($this->theme, $type);
                if ($filename) {
                    $view->$method($asset->baseUrl . '/' . $filename, [
                        'depends' => BrowserNotificationAsset::className()
                    ]);
                }
            }
        }
        // Register timeago i18n file
        if ($filename = BrowserNotificationAsset::getTimeAgoI18n($this->timeAgoLocale)) {
